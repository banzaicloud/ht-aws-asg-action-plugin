...
func updateLaunchConfig(session *session.Session, name string) error {
	...
	recommendations, err := recommender.RecommendSpotInstanceTypes(*session.Config.Region, aws.StringValueSlice(group.AvailabilityZones), baseInstanceType)
	if err != nil {
		log.WithFields(logrus.Fields{
			"autoScalingGroup": name,
